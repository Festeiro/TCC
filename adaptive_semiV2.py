...
from skfeature.function.similarity_based import lap_score as LapScore
from skfeature.utility import construct_W as Construct_W
from skmultiflow.core.base import BaseSKMObject, ClassifierMixin
from skmultiflow.utils import get_dimensions
from sklearn.neighbors import KNeighborsClassifier
import os

kwargs_W = {"metric": "euclidean", "neighbor_mode": "knn", "weight_mode": "heat_kernel", "k": 1, 't': 1}

class AdaptiveFS(BaseSKMObject, ClassifierMixin):
    ...
    def __init__(self,
        ...
        self._features_temp = None
        self._features_booster = None
    ...
    def partial_fit(self, X, y, classes=None, sample_weight=None):
        ...
        # LapScore.lap_score(X, W=W)
        # score = LapScore.lap_score(X, W=W)
        # idx = LapScore.feature_ranking(score)
        # X = X[:, idx[0:num_features]]
    ...
    def _train_on_mini_batch(self, X, y):
        if self._first_train:
            ...
            score = LapScore.lap_score(X, W=W)
            idx = LapScore.feature_ranking(score)
            self._features_index = idx[0:self.total_attributes]
            self._first_train = False

        X = X[:, self._features_index[0:self.total_attributes]]
        ...
        if inside_pre_train >= self._pre_train:
            #fazer FS e salvar array de selected features aqui
            # W = Construct_W.construct_W(X, **kwargs_W)
            # score = LapScore.lap_score(X, W=W)
            # features_temp = LapScore.feature_ranking(score)
            # X_temp = X[:, features_temp[0:num_features]]
            # self._features_temp = features_temp
        ...
        if self._count_buffer >= self._max_buffer:
            ...
            # self._features_booster = self._features_temp
    ...
    def predict(self, X):
        if self._first_run:
            self.total_attributes = np.shape(X)[1]
            num_features = int(self.total_attributes * 0.35)
            print("after feature selection: ", num_features)
            self.total_attributes = num_features
